...
namespace autoinput::ui
{
    namespace
    {
        ...
        class ImGuiRaylibBackEnd : public IUiBackend
        {
        public:
            using IUiBackend::IUiBackend;

            void init() override;
            void shutdown() override;
            void newFrame() override;
            void render() override;
        [[nodiscard]] bool windowShouldClose() const override;

            double m_time{ 0.0 };
            Texture2D m_fontTexture{ 0 };
        };

        void ImGui_ImplRaylib_RenderDrawData(ImDrawData* draw_data)
        {
            if (!draw_data || !draw_data->Valid || draw_data->CmdLists.Size == 0)
            {
                return;
            }
    ...
    {
        return std::make_unique<ImGuiRaylibBackEnd>();
    }

    void ImGuiRaylibBackEnd::init()
    {
    ...
    void ImGuiRaylibBackEnd::shutdown()
    {
        if (m_fontTexture.id != 0)
        {
            UnloadTexture(m_fontTexture);
            m_fontTexture = { 0 };
        }
    ...
    void ImGuiRaylibBackEnd::newFrame()
    {
        BeginDrawing();
        ...
        io.DeltaTime = m_time > 0.0 ? static_cast<float>(currentTime - m_time) : (float)(1.0f / 60.0f);
        m_time = currentTime;
    ...
    bool ImGuiRaylibBackEnd::windowShouldClose() const
    {
