...
namespace autoinput
{
    ...
    SequenceRecorder::SequenceRecorder(std::string name, IPlatformBackend* backend)
        : m_backend(backend)
    {
        m_sequence.name = std::move(name);
    }
    ...
    void SequenceRecorder::start()
    {
        ...
        Logger::info("Recording started for sequence: {}\n", m_sequence.name);
    }

    void SequenceRecorder::stop()
    {
        m_state = State::Finished;
        Logger::info("Recording finished for sequence: {}\n", m_sequence.name);
    }

    void SequenceRecorder::cancel()
    {
        m_state = State::Cancelled;
        Logger::info("Recording cancelled for sequence: {}\n", m_sequence.name);
    }

    void SequenceRecorder::addEvent(RecordedEvent&& event)
    {
        auto now = std::chrono::steady_clock::now();
        auto delay = std::chrono::duration_cast<std::chrono::milliseconds>(now - m_lastEventTime);
        event.delay = std::to_string(delay.count()) + "ms";
        m_sequence.events.emplace_back(std::move(event));
        m_lastEventTime = now;
    }

    void SequenceRecorder::recordKeyEvent(const Key& key, bool isDown)
    {
    ...
    void SequenceRecorder::recordMouseEvent(const Mouse& mouse, bool isDown, int32_t x, int32_t y)
    {
        if (m_state != State::Recording) return;
    ...
    void SequenceRecorder::recordMouseMove(int32_t x, int32_t y)
    {
        if (m_state != State::Recording) return;
