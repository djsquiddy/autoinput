...
namespace autoinput
{
    ...
    SequenceHandler::SequenceHandler(const SequenceHandler& rhs) = default;
    ...
    void SequenceHandler::press()
    {
        ...
        if (bool expected = false; m_isActive.compare_exchange_strong(expected, true))
        {
            ...
            m_autoclickerThread = std::jthread([this](const std::stop_token& stoken) { playback(stoken); });
        }
    }
    ...
    void SequenceHandler::playback(const std::stop_token& stoken)
    {
        ...
        do
        {
            for (const auto& event : m_sequence.events)
            {
                ...
                if (!m_isActive || stoken.stop_requested())
                {
                    break;
                }
                ...
                case RecordedEventType::KeyDown:
                    if (event.key)
                    {
                        m_backend->keyDown(Key::fromString(*event.key));
                    }
                    break;
                case RecordedEventType::KeyUp:
                    if (event.key)
                    {
                        m_backend->keyUp(Key::fromString(*event.key));
                    }
                    break;
                case RecordedEventType::MouseDown:
                    if (event.button)
                    {
                        m_backend->mouseDown(Mouse::fromString(*event.button));
                    }
                    break;
                case RecordedEventType::MouseUp:
                    if (event.button)
                    {
                        m_backend->mouseUp(Mouse::fromString(*event.button));
                    }
                    break;
                case RecordedEventType::MouseMove:
                    if (event.x && event.y)
                    {
                        m_backend->moveMouseTo(*event.x, *event.y);
                    }
                    break;
                default:
                    break;
