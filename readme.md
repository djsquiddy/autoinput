...
- [UI Graph Document Model](docs/ui-graph-model.md) — Internal graph model, sequence adapter/compiler, config graph adapter/viewer, configuration diagnostics, topological validation engine, fallback Dear ImGui viewer, sequence graph editor, and optional imnodes / imgui-node-editor rendering backends for future visual editors.
