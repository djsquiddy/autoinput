...
- [UI Graph Document Model](docs/ui-graph-model.md) — Internal graph model, sequence adapter/compiler, config graph adapter/viewer, configuration diagnostics, topological validation engine, fallback Dear ImGui viewer, sequence graph editor, custom draw-list canvas prototype, optional imnodes / imgui-node-editor backends, and layout metadata persistence for future visual editors.
